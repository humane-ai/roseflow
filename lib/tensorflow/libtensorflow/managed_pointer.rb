require "tensorflow/libtensorflow/data_converters/type_safety"
  ...
  class ManagedPointer < FFI::AutoPointer
    ...
    class << self
      ...
      def release(pointer)
        unless pointer.null?
          ...
          TensorFlow::LibTensorFlow::API.public_send(:"delete_#{type}", pointer)
        end
      end

      def retain(pointer)
        unless pointer.null?
          TensorFlow::LibTensorFlow::API.public_send(:"#{type}_add_ref", pointer)
        end
      end
    ...
    def free
      unless null?
        ...
        TensorFlow::LibTensorFlow::API.public_send(:"delete_#{type}", self)
      end
    end
