...
RSpec.describe Roseflow::ActiveGraph::Input do
  describe "Creating an input" do
    ...
    context "Placeholder" do
      class MyGraph < Roseflow::ActiveGraph::Base
      end

      let(:graph) { MyGraph.new }
      let(:with_placeholder) { input_attr.merge(placeholder: true) }
      let(:without_graph) { input_attr.merge(placeholder: true) }

      it "creates a new input and a placeholder" do
        input = described_class.new(with_placeholder, graph)
        expect(input).to be_a described_class
  ...
  describe "Class Methods" do
    describe ".build" do
      ...
      it "creates a new instance of the class with given arguments" do
        expect(described_class).to receive(:new).with(arguments, nil)
