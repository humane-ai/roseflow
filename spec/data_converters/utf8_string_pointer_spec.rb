...
RSpec.describe Roseflow::Tensorflow::UTF8StringPointer do
  describe ".to_native" do
    it "returns a memory pointer containing the given string" do
      pointer = Roseflow::Tensorflow::UTF8StringPointer.to_native("coolio", nil)
      expect(pointer.read_string).to eq "coolio"
    end

    it "returns nil when given nil" do
      pointer = Roseflow::Tensorflow::UTF8StringPointer.to_native(nil, nil)
      expect(pointer).to be_nil
    end

    it "converts strings to UTF-8" do
      string = "åäö".encode("ISO-8859-1")
      pointer = Roseflow::Tensorflow::UTF8StringPointer.to_native(string, nil)
    ...
    it "raises an error when given a non-string" do
      expect { Roseflow::Tensorflow::UTF8StringPointer.to_native({}, nil) }
  ...
  describe ".from_native" do
    it "returns an empty string if given a null pointer" do
      value = Roseflow::Tensorflow::UTF8StringPointer.from_native(FFI::Pointer::NULL, nil)
      expect(value).to be_nil
    end

    it "returns the string value of the string pointer" do
      pointer = FFI::MemoryPointer.from_string("hey")
      value = Roseflow::Tensorflow::UTF8StringPointer.from_native(pointer, nil)
    ...
    it "forces the encoding to UTF-8" do
      ...
      value = Roseflow::Tensorflow::UTF8StringPointer.from_native(string, nil)
